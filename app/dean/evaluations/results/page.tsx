...
export default function DeanEvaluationResultsPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(() => {
      ...
      setStudentData({})
      setPage(0)
      fetch(`/api/dean/evaluation-results?periodId=${selectedPeriod}&_=${Date.now()}`)
        .then((r) => r.json())
        .then((data) => {
  ...
  const downloadPDF = useCallback(async () => {
    ...
    doc.setFontSize(16)
    doc.text("Department Evaluation Results", pageW / 2, 15, { align: "center" })
    doc.setFontSize(10)
    doc.text(`Period: ${periodName}`, pageW / 2, 22, { align: "center" })
    doc.setFontSize(8)
    doc.text(`Dept Average: ${deptAvg.toFixed(2)}  |  Total Respondents: ${deptTotalResp}  |  Generated: ${new Date().toLocaleDateString()}`, pageW / 2, 28, { align: "center" })
    ...
    doc.save("department-evaluation-results.pdf")
    ...
    <div className="pb-12">
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-200 z-20">
        <div className="max-w-[1400px] mx-auto px-6 h-14 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-primary">Evaluation Dashboard</h1>
            <p className="text-[11px] text-tertiary">Department results overview</p>
          </div>
      ...
      {loading ? (
        <p className="text-sm text-tertiary text-center pt-20">Loading...</p>
      ) : results.length === 0 ? (
          ...
          {departmentSummary && (
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div className="text-xs text-tertiary mt-1">Department Average</div>
              </div>
              <div className="bg-white rounded-xl border border-slate-200 p-5">
...
function DetailPanel({
  ...
  return (
    <div className="mt-6">
      ...
      {loading ? (
        <p className="text-sm text-tertiary text-center py-10">Loading {result.totalRespondents} responses...</p>
      ) : students.length === 0 ? (
