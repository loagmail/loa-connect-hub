...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!dept) return NextResponse.json({ results: [], facultyNames: {} })
    ...
    for (const [facId, evaluationIds] of facultyEvalMap) {
      const u = userMap.get(facId)
      if (u?.departmentId !== dept.id) continue

      facultyNames[facId] = u?.name || facId
      ...
      for (const [catName, avg] of Object.entries(catAverages)) {
        ...
        if (col) {
          row[col] = Math.round(avg * 100) / 100
        }
      }
