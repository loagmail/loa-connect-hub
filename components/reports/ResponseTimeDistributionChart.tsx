...
export function ResponseTimeDistributionChart({ distribution }: ResponseTimeDistributionChartProps) {
  ...
  if (distribution.length === 0 || distribution.every((d) => d.count === 0)) {
    return (
      <div className="rounded-2xl border border-default/70 bg-surface p-6 shadow-sm">
        <h3 className="text-sm font-bold text-primary mb-4">Response Time Distribution</h3>
  ...
  return (
    <div className="rounded-2xl border border-default/70 bg-surface p-6 shadow-sm transition-all duration-200 hover:shadow-md">
      <h3 className="text-sm font-bold text-primary mb-1">Response Time Distribution</h3>
      <p className="text-xs text-tertiary mb-5">How quickly consultations are responded to</p>
          ...
          return (
            <div key={bucket.label}>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="font-medium text-secondary">{bucket.label}</span>
                <span className="font-mono text-tertiary ml-2">{bucket.count}</span>
              </div>
              <div className="w-full h-5 bg-surface rounded-full overflow-hidden relative">
                <div
