...
export function SegmentedControl<T extends string>({ options, selected, onSelect }: { options: { key: T; label: string; disabled?: boolean }[]; selected: T; onSelect: (key: T) => void }) {
  return (
    <div className="flex gap-1 p-1 bg-surface-tertiary rounded-xl overflow-x-auto scrollbar-hide">
      {options.map((opt) => (
        <button
          key={opt.key}
          onClick={() => { if (!opt.disabled) onSelect(opt.key) }}
